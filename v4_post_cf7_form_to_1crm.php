...
class v4_post_cf7_form_to_1crm {
	...
	public function wpcf7_before_send_mail( WPCF7_ContactForm $wpcf7 ) {
		...
		$url = $wpcf7->additional_setting( 'onecrm_url', true );
		if ( count( $url ) ) {
			$url = $url[0];
		} else {
		...
		$return_field = $wpcf7->additional_setting( 'onecrm_return_field', true );
	...
	function menu() {
		add_options_page( "CRM Lead Capture", "CRM Lead Capture", 'manage_options', "v4-1crm-cf7", array(
				$this,
				'admin_page'
			)
		);
