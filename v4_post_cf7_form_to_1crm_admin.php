...
<div class="wrap">
	...
	<form method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI'] ); ?>">
		...
		<?php if ( $active_tab == "simple" ): ?>
			<h3>Required</h3>
			<p>These are the most basic settings you must configure. Without these, you won't be able to use 1CRM Lead
				Capture.</p>
			<table class="form-table">
				<tbody>
				<tr>
					<th scope="row" valign="top"><label for="<?php echo $this->get_field_name( 'lc_uri' ); ?>">Lead
							Capture URI</label></th>
					<td>
						<input type="text" name="<?php echo $this->get_field_name( 'lc_uri' ); ?>"
						       id="<?php echo $this->get_field_name( 'lc_uri' ); ?>"
				...
				<tr>
					<th scope="row" valign="top"><label for="<?php echo $this->get_field_name( 'campaign_id' ); ?>">Related
							Campaign</label></th>
					<td>
						<input type="text" name="<?php echo $this->get_field_name( 'campaign_id' ); ?>"
						       id="<?php echo $this->get_field_name( 'campaign_id' ); ?>"
				...
				<tr>
					<th scope="row" valign="top"><label
							for="<?php echo $this->get_field_name( 'assigned_user_id' ); ?>">Assigned User</label></th>
					<td>
						<input type="text" name="<?php echo $this->get_field_name( 'assigned_user_id' ); ?>"
						       id="<?php echo $this->get_field_name( 'assigned_user_id' ); ?>"
						       value="<?php echo join( ';', (array) $v4ContactForm->get_setting( 'assigned_user_id' ) ); ?>"/><br/>
						The ID of the user that captured leads should be assigned to.
					</td>
				</tr>
				</tbody>
			</table>
		...
		<?php else: ?>
			...
			<h4>The Form</h4>
			<p>
				You can create forms from within <em>1CRM->Campaigns->Create Lead Form</em> that contain all possible
				fields.<br/>
				These forms need to be reworked for use with contact form 7 of course, as this plugin uses special tags.<br/>
				A simple form could look like this:</p>
